    ...
    >
      <div className="flex items-center gap-3 px-2">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">Tính điểm CTV</p>
        </div>
      </div>
    ...
    <header className="sticky top-0 z-30 border-b bg-background/95 backdrop-blur">
      <div className="flex h-16 items-center justify-between gap-3 px-4 sm:px-6">
        <div className="flex items-center gap-3">
          <Button variant="outline" size="icon" className="lg:hidden" onClick={onMenu} aria-label="Mở menu">
            <Menu className="h-5 w-5" />
          </Button>
          <div>
            <h1 className="text-lg font-semibold">Hệ thống tính điểm CTV</h1>
            <p className="hidden text-sm text-muted-foreground sm:block">Quản lý điểm, KPI và lương theo mạng lưới giới thiệu</p>
          </div>
        </div>
  ...
  return (
    <div className="min-h-screen bg-muted/35 text-foreground">
      <div className="lg:flex">
        ...
        {open && <button className="fixed inset-0 z-30 bg-black/40 lg:hidden" onClick={() => setOpen(false)} aria-label="Đóng menu" />}
        <div className="min-w-0 flex-1">
          ...
          <main className="mx-auto max-w-7xl p-4 sm:p-6">
            <div className="mb-6 flex flex-col justify-between gap-3 sm:flex-row sm:items-end">
              ...
              <Badge variant="secondary">API Cloudflare D1</Badge>
            </div>
